...
// Adapted to TypeScript for this project.

import { useMemo, useEffect, useLayoutEffect, useRef, useState, type ReactNode } from 'react'
import { motion, useMotionValue, useTransform, animate, type MotionValue, type Easing } from 'motion/react'
...
interface OrbitImagesProps {
  images?: string[]
  // Optional per-image scale (parallel to `images`), 1 = default size.
  // Lets a logo render smaller inside its orbit frame without shrinking
  // the frame itself.
  imageScales?: number[]
  altPrefix?: string
  shape?: OrbitShape
...
export default function OrbitImages({
  ...
  useLayoutEffect(() => {
    ...
    const updateScale = () => {
      if (!node) return
      setScale(node.clientWidth / baseWidth)
    }
    updateScale()
  ...
  const items = images.map((src, index) => {
    ...
    return (
      <span key={src} className="orbit-image-frame">
        <img
          src={src}
  ...
  return (
    ...
    >
      ...
      {centerContent && (
        <div className="absolute inset-0 flex items-center justify-center z-10">{centerContent}</div>
      )}
    </div>
  )
}
