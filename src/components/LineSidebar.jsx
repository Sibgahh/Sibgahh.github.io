...
const LineSidebar = ({
  ...
  // Sync with external active index (e.g. driven by IntersectionObserver)
  useEffect(() => {
      ...
      >
        {items.map((label, index) => (
          ...
          >
            ...
            <span className="relative inline-flex items-baseline gap-2 leading-[1.3] [color:color-mix(in_srgb,var(--accent-color)_calc(var(--effect,0)*100%),var(--text-color))] [font-size:var(--font-size)] [transform:translateX(calc(var(--effect,0)*var(--max-shift)))]">
              {showIndex && (
                <span className="inline-block min-w-[1.4rem] font-mono text-[0.8em] font-semibold tracking-wider [opacity:calc(0.55+var(--effect,0)*0.45)]">
